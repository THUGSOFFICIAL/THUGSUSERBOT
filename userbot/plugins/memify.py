"""
Created by @mrconfused and @sandy1709 used on Thugbot
memify plugin
"""
import asyncio
import os
...
async def memes(cat):
    ...
    if catinput:
        ...
        await edit_or_reply(
            cat, "```You didn't tell me what to write please gib some text to write```"
        )
        return
...
async def memes(cat):
    ...
    await borg.send_file(cat.chat_id, outputfile, force_document=False, reply_to=catid)
    await cat.delete()
